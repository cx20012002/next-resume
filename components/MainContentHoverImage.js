...
function MainContentHoverImage({link, image}) {
    return (
        <Link href={link} as={link}
              className={`w-full lg:h-[720px] md:h-[520] h-[480px] flex flex-col items-start rounded-3xl overflow-hidden relative group/img hover:rounded-[40px] duration-300`}>
            <Image placeholder={"blur"} src={image} alt={"service"} className={"w-full h-full object-cover"}/>
